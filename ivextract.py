...
def run_invoice_extractor():
    ...
    uploaded_file = st.file_uploader("Upload Invoice Image", type=['jpg', 'jpeg', 'png'], label_visibility="visible", help="Upload a JPG, JPEG, or PNG invoice image")
    ...
    if uploaded_file:
        ...
        """, unsafe_allow_html=True)
        user_input = st.text_input("Enter your question", key="input", label_visibility="collapsed", help="Type a question about the invoice")
        st.markdown("</div>", unsafe_allow_html=True)
        ...
        with col2:
            analyze_button = st.button("Analyze Invoice", width="stretch")
